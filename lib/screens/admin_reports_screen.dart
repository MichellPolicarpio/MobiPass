  ...
  Future<void> _loadReports() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('$serverUrl/api/reports/all'),
        headers: {
      ...
      print('URL: $serverUrl/api/reports/all');
  ...
  Future<void> _updateReportStatus(String reportId, String newStatus) async {
    try {
      final response = await http.put(
        Uri.parse('$serverUrl/api/reports/$reportId/status'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ${widget.user.token}',
        },
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Error al actualizar estado: ${response.body}'),
              backgroundColor: Colors.red,
            ),
          );
        }
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Card(
                        ...
                        child: ExpansionTile(
                          title: Text(
                            'Reporte #${report.id?.substring(0, 8) ?? 'Nuevo'}',
                            style: const TextStyle(
                              fontWeight: FontWeight.bold,
                            ),
                          ),
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ElevatedButton(
                                        onPressed: report.status.toLowerCase() != 'pendiente'
                                            ? () => _updateReportStatus(report.id!, 'pendiente')
                                            : null,
                                        style: ElevatedButton.styleFrom(
                                      ...
                                      ElevatedButton(
                                        onPressed: report.status.toLowerCase() != 'en proceso'
                                            ? () => _updateReportStatus(report.id!, 'en proceso')
                                            : null,
                                        style: ElevatedButton.styleFrom(
                                          backgroundColor: Colors.blue,
                                          foregroundColor: Colors.white,
                                      ...
                                      ElevatedButton(
                                        onPressed: report.status.toLowerCase() != 'resuelto'
                                            ? () => _updateReportStatus(report.id!, 'resuelto')
                                            : null,
                                        style: ElevatedButton.styleFrom(
                                          backgroundColor: Colors.green,
                                          foregroundColor: Colors.white,
